...
const SupportAdminList = ({ onSelectAdmin, onClose }) => {
    const { user } = useAuth();
    ...
    const fetchAdmins = async () => {
        try {
            ...
            if (response.success) {
                ...
                const filteredAdmins = response.data.filter(admin => admin._id !== user?._id);
        ...
        <div className="fixed bottom-16 sm:bottom-20 right-2 sm:right-4 w-[95vw] sm:w-80 bg-white rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden border border-gray-200 z-50 animate-in slide-in-from-bottom-5">
            ...
            <div className="p-3 sm:p-4 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white flex justify-between items-center">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-sm sm:text-base">Support Team</h3>
                </div>
                <button
                    onClick={onClose}
            ...
            <div className="max-h-[50vh] sm:max-h-96 overflow-y-auto">
                {loading ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <button onClick={fetchAdmins} className="text-emerald-600 hover:text-emerald-700 text-sm font-medium">
                            Try Again
                        </button>
                    </div>
                ) : admins.length === 0 ? (
                    <div className="p-8 text-center">
                        ...
                        <p className="text-gray-500 text-sm">No support agents available.</p>
                        ...
                        {admins.map(admin => (
                            ...
                            >
                                <div className="relative flex-shrink-0">
                                    <img
                                        src={admin.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(admin.name)}&background=${admin.isSuperAdmin ? '9333ea' : '10b981'}&color=fff`}
                                        alt={admin.name}
                                    ...
                                    />
                                    {admin.isActive && (
                                        <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 border-2 border-white rounded-full" title="Online" />
                                    )}
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-gray-500 truncate">
                                        {admin.bio || (admin.isSuperAdmin ? 'Senior Support' : 'Here to help!')}
                                    </p>
                                </div>
